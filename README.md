<div id="top"></div>

<br />
<div align="center">

<!-- 🔰 BADGES -->
![GitHub repo size](https://img.shields.io/github/repo-size/codewith-salman/twitter-sidebar-menu)
![GitHub stars](https://img.shields.io/github/stars/codewith-salman/twitter-sidebar-menu?style=social)
![GitHub forks](https://img.shields.io/github/forks/codewith-salman/twitter-sidebar-menu?style=social)

[![Live Demo](https://img.shields.io/badge/Live%20Demo-View%20Project-brightgreen?style=for-the-badge&logo=vercel)](https://twitter-sidebar-menu.vercel.app/)

[![GitHub Follow](https://img.shields.io/github/followers/codewith-salman?style=social)](https://github.com/codewith-salman)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Muhammad%20Salman%20Arshad-blue?style=social&logo=linkedin)](https://www.linkedin.com/in/muhammad-salmanarshad/)
[![Figma](https://img.shields.io/badge/Figma-codewithsalman-black?style=social&logo=figma)](https://www.figma.com/@codewithsalman)
[![Email](https://img.shields.io/badge/Email-msalmanwebdev%40gmail.com-red?style=social&logo=gmail)](mailto:msalmanwebdev@gmail.com)

<br />

<!-- 🔰 PROJECT LOGO -->
<img src="Twitter%20Sidebar%20Menu/images/twitter.png" alt="Twitter Sidebar Menu Logo" width="120"/>

<br />
<br />

<h1 align="center">🐦 Twitter Sidebar Menu</h1>

A clean, modern, and fully responsive **Twitter-Inspired Sidebar Navigation UI**  
designed to replicate the familiar Twitter sidebar experience with smooth interaction.

<a href="https://twitter-sidebar-menu.vercel.app/"><strong>➥ Live Demo</strong></a>

</div>

---

<!-- TABLE OF CONTENTS -->
<details>
  <summary>📑 Table of Contents</summary>
  <ol>
    <li><a href="#about-the-project">About The Project</a></li>
    <li><a href="#features">Features</a></li>
    <li><a href="#built-with">Built With</a></li>
    <li><a href="#contact">Contact</a></li>
  </ol>
</details>

---

## 📖 About The Project

<img src="Twitter%20Sidebar%20Menu/images/desktop.png" alt="Twitter Sidebar Menu Screenshot" width="100%" />

The **Twitter Sidebar Menu** is a UI-focused navigation component inspired by Twitter’s sidebar design, built for dashboards, social platforms, and modern web applications.

The project focuses on:
- Icon-based navigation  
- Clean spacing and alignment  
- Familiar social-media UX patterns  
- Responsive and reusable layout  

This project is ideal for:
- Social media UI clones  
- Dashboard sidebar navigation  
- Web app UI components  
- Front-end layout and spacing practice  
- UI component portfolio projects  

This is a **front-end only project**, created for UI practice, learning, and portfolio showcasing — no backend integration included.

<p align="right">(<a href="#top">back to top</a>)</p>

---

## ✨ Features

- Twitter-inspired sidebar navigation UI  
- Fully responsive layout  
- Icon-based menu items  
- Clean and minimal design  
- Smooth hover and active states  
- Lightweight and reusable component  

<p align="right">(<a href="#top">back to top</a>)</p>

---

## 🛠️ Built With

This project is built using:

- **HTML5**  
- **CSS3**  
- **JavaScript (Vanilla)**  

<p align="right">(<a href="#top">back to top</a>)</p>

---

## 📬 Contact

**Muhammad Salman Arshad**

- 💼 **LinkedIn:** https://www.linkedin.com/in/muhammad-salmanarshad/  
- 🎨 **Figma:** https://www.figma.com/@codewithsalman  
- 📧 **Email:** [msalmanwebdev@gmail.com](mailto:msalmanwebdev@gmail.com)

<p align="right">(<a href="#top">back to top</a>)</p>
